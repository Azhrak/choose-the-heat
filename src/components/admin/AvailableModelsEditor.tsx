...
export function AvailableModelsEditor({
	...
	const providers: ProviderMetadata[] =
		category === "ai" ? getAllTextProviders() : getAllTTSProviders();

	// Get default model for a provider from settings
	const getDefaultModel = (providerId: string): string | null => {
		if (!allSettings) return null;
		const settingKey = `${category}.${providerId}.default_model`;
	...
	const handleToggleModel = (providerId: string, model: string) => {
		...
		let updated: ModelsData;
		if (isSelected) {
			// Remove model
			updated = {
				...data,
				[providerId]: currentModels.filter((m) => m !== model),
			};
		} else {
			// Add model
			updated = {
	...
	const handleSetDefaultModel = (providerId: string, model: string) => {
		...
		// If the model is not in the available models list, add it
		if (!currentModels.includes(model)) {
			const updated = {
				...data,
				[providerId]: [...currentModels, model],
			};
			changes[`${category}.available_models`] = JSON.stringify(updated);
		}
